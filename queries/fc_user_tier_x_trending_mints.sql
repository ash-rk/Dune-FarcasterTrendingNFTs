WITH reaction_summary AS (
  ...
  WHERE
    ...
    AND c.created_at >= CURRENT_DATE - INTERVAL '8' DAY
...
SELECT
  fd.fid_active_tier_name,
  bmf.nft_contract_address AS token_address,
  fc_mints.token_type,
  MAX(fc_mints.nft) AS nft_name,
  MAX(fc_mints.unique_mints) AS unique_mint_number,
